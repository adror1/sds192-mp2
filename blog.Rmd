...
```{r}
## Function that joins for, against, and candidates table
cand_info <- function(for_table, against_table, cand_table) {
  for_table %>%
    full_join(against_table, by = "cand_name") %>%
    left_join(cand_table, by = "cand_name") %>%
    select(cand_id, cand_name, cand_party_affiliation, cand_state, cand_election_yr, cand_office, cand_office_district, total_for, total_against)
}

## Example
CA_cand_info <- cand_info(for_table = CA_house_cand_for, against_table = CA_house_cand_against, cand_table = CA_house)
CA_cand_info
...
candidates_full <- house_elections %>%
  ...
  filter(state == "CA", transaction_amt > 0, general_votes > 0, transaction_type == "24A")
